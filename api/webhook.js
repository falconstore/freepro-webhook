    ...
    if (Event === 'Purchase_Order_Confirmed' && Data?.Buyer?.Email) {
      ...
      try {
        ...
        if (emailSent) {
          ...
          await sendErrorNotification(customer_email, 'Falha no envio de email');
        }
        ...
        await sendErrorNotification(customer_email, `Erro Firebase: ${firebaseError.message}`);
...
// Email de renovação para usuários existentes
...
// Email de notificação de erro para suporte
async function sendErrorNotification(customerEmail, errorMessage) {
  try {
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      method: 'POST',
      headers: {
